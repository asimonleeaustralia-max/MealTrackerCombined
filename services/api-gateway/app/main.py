...
# Backends to wake on /api/warmup (vision stays cold until a photo is analyzed).
_WARMUP_SERVICES: tuple[tuple[str, str], ...] = (
    ("auth", "auth_service_url"),
    ("meal", "meal_service_url"),
    ("nutrition", "nutrition_service_url"),
)
...
def create_app() -> FastAPI:
    ...
    @app.get("/api/warmup", tags=["meta"])
    async def warmup() -> dict[str, object]:
        """Ping every core backend in parallel so cold Container App replicas spin up."""

        async def _ping(name: str, attr: str) -> tuple[str, str]:
            base = getattr(settings, attr).rstrip("/")
            try:
                rsp = await _client.get(f"{base}/healthz", timeout=45.0)
                return name, "ok" if rsp.status_code == 200 else "degraded"
            except httpx.HTTPError:
                log.warning("Warmup ping failed for %s", name)
                return name, "error"

        results = await asyncio.gather(
            *(_ping(name, attr) for name, attr in _WARMUP_SERVICES)
        )
