...
/** Wake backend services (parallel cold-start) and keep them alive while the tab is open. */
const Warmup = {
  _inFlight: null,
  _keepaliveTimer: null,
  KEEPALIVE_MS: 10 * 60 * 1000, // stay under scale-to-zero while tab is open; light on requests

  ping() {
    if (!this._inFlight) {
      this._inFlight = fetch(`${API}/warmup`)
        .catch(() => {})
  ...
  startKeepalive() {
    this.stopKeepalive();
    this._keepaliveTimer = setInterval(() => this.ping(), this.KEEPALIVE_MS);
  ...
  _onVisibility() {
    if (document.visibilityState === "visible") Warmup.ping();
  },
};
...
(async function bootstrap() {
  const warmupPromise = Warmup.ping();
  ...
  if (tokens.access) {
    const waitTimer = setTimeout(() => {
      const wp = document.getElementById("login-waiting-page");
